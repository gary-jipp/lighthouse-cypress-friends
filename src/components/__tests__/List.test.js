...
describe('List Tests', () => {
  ...
  it("renders with data and renders a list", () => {
    const { container, getByRole } = render(<List items={data} />);
  ...
  it("renders expected number of items", () => {
    ...
    // console.log(prettyDOM(container));
    //  const list = screen.getAllByRole("list"); // oops!  Only list
  ...
  it("calls onClick with the correct uid when an item clicked", () => {
    ...
    const { container } = render(<List items={data} deleteItem={onClick} />);
    // console.log(prettyDOM(container));
